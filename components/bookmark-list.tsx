...
// Plain card used inside DragOverlay — no useDraggable, just visual clone
export function BookmarkDragOverlay({ bookmark }: { bookmark: BookmarkWithClient }) {
  const faviconUrl = getFaviconUrl(bookmark.url)
  return (
    <div className="flex items-center gap-3 bg-white border border-[#2563EB]/30 rounded-[12px] shadow-2xl p-4 pl-7 w-[560px] max-w-full opacity-95 rotate-1">
      <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-[#F3F4F6] border border-[#E5E7EB] flex items-center justify-center overflow-hidden">
        {faviconUrl && <img src={faviconUrl} alt="" className="w-5 h-5" />}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[15px] font-semibold text-[#111827] truncate">{bookmark.title}</p>
        <p className="text-[13px] text-[#6B7280] truncate">{getDomainFromUrl(bookmark.url)}</p>
      </div>
    </div>
  )
}

// Individual draggable bookmark row — extracts useDraggable so hooks are called at top level
function DraggableBookmarkRow({
  ...
  return (
    ...
    >
      {/* Drag grip — listeners only on the grip, not the whole card */}
      {!isEditing && !isTemp && (
        <div
          {...listeners}
          className="absolute left-1.5 top-1/2 -translate-y-1/2 p-1 rounded cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity duration-150 text-[#9CA3AF] hover:text-[#6B7280] touch-none"
